...
router.post('/saveReservation2', (req, res) => {
    ...
    let values = [];
    for (const element of body.picked) {
        values.push([element, body.name, body.phone, body.adult, body.baby, body.dog, body.barbecue, body.studentEvent, body.price, body.priceOption, body.revisit, body.wholeUse, "N"]);
    }
    connection.query('INSERT INTO reservation2 (date, name, phone, adult, baby, dog, barbecue, student_event, price, price_option, revisit, whole_use, confirm) VALUES ?', [values], (err, data) => {
        res.send(data);
